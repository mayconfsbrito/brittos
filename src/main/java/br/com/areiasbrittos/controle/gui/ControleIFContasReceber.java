 ...
 */
public class ControleIFContasReceber {

    /*
     * Variáveis da classe
     */
    private InternalFrameContasReceber frame;
    private DecimalFormat df = new DecimalFormat("###,##0.00");

    public ControleIFContasReceber(InternalFrameContasReceber frame) {
        this.frame = frame;
    }
    ...
    public void inicializa() {
        ...
        //Limita a quantidade de caracteres possíveis
        ...
        frame.textMensagem.setText("Aqui são gerenciadas as contas a receber.");
    ...
    public void verificaAcessos() {
        ...
        if (!FramePrincipal.acessos.get(FramePrincipal.PERMISSAO_ALTERA_CONTAS_RECEBER).isPermissao()) {
    ...
    public void guiAtiva(boolean quitada) {
        ...
        if (quitada) {
            ...
            frame.textMensagem.setText("Esta conta já está quitada/recebida.");
        } else {
            EnabledGUIElements.enabledJComponent(frame.textDescricao, frame.textObservacao,
                    frame.textValor, frame.textVencimento, frame.buttonCadastrar, frame.buttonAlterar,
                    frame.buttonQuitar);
            frame.textMensagem.setText("Esta conta ainda não foi quitada/recebida.");
        }
    ...
    public void tabelaPreencheGUI() {
        ...
        try {
            ...
            List<Contasreceber> consulta = AbstractDAO.consultar("Contasreceber", "idConta=" + cod);

            if (consulta.size() > 0) {
                Contasreceber c = consulta.get(0);
                ...
                if (c.getPesagem() != null) {
                    frame.textPesagem.setText(c.getPesagem().getIdPesagem().toString());
                } else {
                    frame.textPesagem.setText("");
                }

                if (c.getVenda() != null) {
                    frame.textVenda.setText(c.getVenda().getIdVenda().toString());
                } else {
                    frame.textVenda.setText("");
                }
                ...
                if (c.isQuitada()) {
                    ...
                    frame.buttonExcluir1.setEnabled(false);
                } else {
                    frame.checkQuitada.setSelected(false);
                    frame.buttonQuitar.setEnabled(true);
                    frame.buttonExcluir1.setEnabled(true);
                }
     ...
     */
    public Contasreceber criaConta(int opcao) {
                ...
                if (vencimento != null) {
                    if (descricao != null && !descricao.trim().equals("")) {
                        if (valor != null) {
                            ...
                            //Verifica se a Conta a pagar está associada a uma pesagem ou venda
                            if (ConsertaBugsGUI.recebeInteiroEmJTextFieldComMascara(frame.textPesagem) != 0) {
                                pesagem = (Pesagem) AbstractDAO.consultar("Pesagem", "idPesagem=" + ConsertaBugsGUI.recebeInteiroEmJTextFieldComMascara(frame.textPesagem)).get(0);
                            } else if (ConsertaBugsGUI.recebeInteiroEmJTextFieldComMascara(frame.textVenda) != 0) {
                                venda = (Venda) AbstractDAO.consultar("Venda", "idVenda=" + ConsertaBugsGUI.recebeInteiroEmJTextFieldComMascara(frame.textVenda)).get(0);
                            }
                            ...
                            //Cria o objeto
                            Contasreceber conta = new Contasreceber(pesagem, venda, descricao, observacao, vencimento, dataPagamento, horaPagamento, valor, quitada);
    ...
    public void limparGUI() {
        ...
        this.buscar();
        new ApagaElementosDaInterface().apagaJTextField(frame.textCodigo, frame.textPesagem, frame.textVenda, frame.textVencimento, frame.textDescricao, frame.textObservacao,
        ...
        EnabledGUIElements.disabledJComponent(frame.buttonAlterar, frame.buttonQuitar, frame.buttonExcluir1);
    ...
    public void listarTodosElementosNaTabela() {
        ...
        List<Contasreceber> list = AbstractDAO.listar("Contasreceber c LEFT JOIN FETCH c.pesagem p LEFT JOIN FETCH c.venda v ", "c.dataVencimento");
     ...
     */
    public void listarPesquisaNaTabela(List<Contasreceber> list) {
    ...
    public void cadastrar() {
        ...
        int opcao = JOptionPane.showOptionDialog(frame, "Tem certeza que deseja efetuar o cadastro da conta a receber?", "Cadastrar", JOptionPane.DEFAULT_OPTION, JOptionPane.QUESTION_MESSAGE, null,
        ...
        if (frame.textCodigo.getText().isEmpty()) {
            ...
            if (this.criaConta(opcao) != null) {
                Contasreceber c = this.criaConta(opcao);
    ...
    public void alterar() {
        ...
        try {
            ...
            if (!frame.textCodigo.getText().equals("")) {
                ...
                Contasreceber c = criaConta(opcao);

                //Realiza a alteração no BD a partir do id*
                if (c != null) {
                    if (AbstractDAO.alterar(c)) {
                        //Registra a transação do usuário
                        DAOTransacao.inserir(new Transacao(FramePrincipal.user, "Alterou a Conta a Receber " + c.getIdConta()));
    ...
    public void excluir() {
        ...
        int opcao = JOptionPane.showOptionDialog(frame, "Tem certeza que deseja excluir esta conta a receber?", "Excluir",
                JOptionPane.DEFAULT_OPTION, JOptionPane.QUESTION_MESSAGE, null, frame.botoesOpcoes, frame.botoesOpcoes[0]);

        //Verifica se a conta cadastrada foi selecionada
        if (ConsertaBugsGUI.recebeInteiroEmJTextFieldComMascara(frame.textCodigo) != 0) {

            Contasreceber conta = this.criaConta(opcao);
            ...
            JOptionPane.showMessageDialog(frame, "Selecione na tabela uma conta a receber já cadastrada para ser excluída.",
                    "Atenção!", JOptionPane.WARNING_MESSAGE);
        }
    }
    ...
    public void quitar() {
        ...
        int opcao = JOptionPane.showOptionDialog(frame, "Você tem certeza que deseja quitar esta conta a receber com o valor de R$" + valor.toString() + "?\n", "Quitar conta a receber",
        ...
        if (opcao == JOptionPane.YES_OPTION) {
            ...
            if (!frame.textCodigo.getText().equals("")) {
                ...
                Contasreceber conta = this.criaConta(opcao);
                Contasreceber contaBd = (Contasreceber) AbstractDAO.consultar("Contasreceber", "idConta=" + conta.getIdConta()).get(0);
