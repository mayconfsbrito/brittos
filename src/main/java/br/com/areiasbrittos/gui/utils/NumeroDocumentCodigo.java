 ...
 * 
* @author Luciano Cargnelutti @version 1.0
 * 
* Exemplo de utilização: JTextField campo = new JTextField(); campo.setHorizontalAlignment(LCTextField.RIGHT); campo.setDocument(new NumeroDocument(7,2));
 */
public class NumeroDocumentCodigo extends PlainDocument {

    private int iMaxLength;
    private int iQtdDec;
    private String sMil;
    private String sDec;
    private boolean zeroEsquerda = false;
    private boolean sinalNegativo = true;

    // Construtor para indicando apenas o tamanho máximo (incluindo o  ponto decimal), será assumido como padrao 2 decimais  
    public NumeroDocumentCodigo(int maxLen, boolean zeroEsquerda, boolean sinalNegativo) {
        this(maxLen + 1, 2, '.', ',', zeroEsquerda, sinalNegativo);
    }

    // Construtor para indicando apenas o tamanho máximo (incluindo o  ponto decimal) e a quantidade de decimais  
    public NumeroDocumentCodigo(int maxLen, int qtdDec, boolean zeroEsquerda, boolean sinalNegativo) {
        this(maxLen + 1, qtdDec, '.', ',', zeroEsquerda, sinalNegativo);
    }
    ...
    public NumeroDocumentCodigo(int maxLen, int qtdDec, char mil, char dec, boolean zeroEsquerda, boolean sinalNegativo) {
        super();
        ...
        this.sinalNegativo = sinalNegativo;
    ...
    public void insertString(int offs, String str, AttributeSet attrSet) throws BadLocationException {
        ...
        if ((texto.length() < this.iMaxLength)
            ...
            /*
             * for (int i = s.length(); i <= iQtdDec; i++) { s.insert(0, "0"); }
             */

            /*
             * // Adicionar o separador da parte decimal if (iQtdDec > 0) { s.insert(s.length() - iQtdDec, sDec); }
             */

            // Adicionar os simbolos de "milhar"  
            /*
             * int iPos = iQtdDec; int nQtdDec = iQtdDec; if (nQtdDec > 0) { iPos++; nQtdDec++; // tratar o separador do decimal } int iQtdMil = 0; while (iPos <
             * s.length() - 1) { iPos++;
             *
             * if ((((iPos - nQtdDec) % 3) == 0) && (iPos < s.length() - iQtdMil)) { s.insert((s.length() - iPos - iQtdMil), sMil); iQtdMil++; } }
             */

            // Caso seja um numero negativo, adicionar o sinal
            if (this.sinalNegativo) {
                if (sinal.equals("-")) {
                    s.insert(0, "-");
                }
            }
